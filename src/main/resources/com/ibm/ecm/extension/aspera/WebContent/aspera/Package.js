...
define([
	...
	var asperaPackage = declare("aspera.Package", LoggerMixin, {
		...
		/**
		 * The encryption password (not supported yet)
		 */
		earPassword: null,
		 ...
		 *
		 * @param callback The function to call back when the send process is started
		...
		/**
		 * An event function called when the send process is started. The response includes the Id of the package being sent.
		 *
		...
		/**
		 * An event function called when the send process is requested to stop.
		 *
		 * @param response The JSON response from the server
		 */
