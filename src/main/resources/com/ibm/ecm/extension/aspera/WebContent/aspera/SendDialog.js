	...
	var sendDialog = declare("aspera.SendDialog", [
		...
		postCreate: function() {
			...
			this.sendButton = this.addButton(this.messages.send_dialog_send_button_label, lang.hitch(this, function() {
				var pkg = this.send(lang.hitch(this, function() {
		...
		/**
		 * Sends the package and calls back when the send process is started or an error is returned.
		 *
		...
		send: function(callback, errorback) {
			...
			var pkg = new Package({
				...
				earPassword: this.earPassword.get("value"),
			...
			var request = pkg.send(lang.hitch(this, function(response) {
