...
require([
	...
	util.stopAll = function() {
		return util._requestService("stopAll", function(response) {
			c.log("Stopped packages:");
			util._onListRetrieved(response.stoppedPackages);
		});
	};

	util.stopAllFromAll = function() {
		return util._requestService("stopAllFromAll", function(response) {
			c.log("Stopped packages:");
			util._onListRetrieved(response.stoppedPackages);
		});
	};
